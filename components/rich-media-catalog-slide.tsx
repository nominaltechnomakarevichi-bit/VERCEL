...
const richMediaFormats = [
  {
    ...
    advantages: ["Возможность показать сразу несколько продуктов в одном баннере", "Пользователь самостоятельно исследует ассортимент, что увеличивает время контакта с брендом", "Эффект «открытия» повышает запоминаемость рекламного сообщения", "Подходит для новых линеек и продуктовых коллекций", "Не требует перехода на сайт для первичного знакомства с ассортиментом"],
    forWhom: "FMCG, Fashion, Beauty, E-commerce, Retail",
